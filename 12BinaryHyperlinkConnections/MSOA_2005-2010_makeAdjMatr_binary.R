...
## FOR EACH YEAR, THE TOP 0.01% MOST CONNECTED HOST1 (outward connections) ARE REMOVED
## A CONNECTION WEIGHT IS NOT VALUED, IT JUST PUTS A BINARY 1 OR 0 FOR IF 2 WEBSITES ARE CONNECTED
# this is to try and remove the effects of large numbers of hyperlinks, because what does that really tell us
# apart from skewing the data and making it really heavy tailed
...
# VERSION 3 EDITS
#################################
websites_exclude = read.csv("websites_excluded0.01.csv", header=TRUE)
# apply the "get_host" function to create a new column 
websites_exclude$hostshort <- get_host(websites_exclude$host)
websites_exclude$hostshort <- tolower(websites_exclude$hostshort)
...
for (year in 2005:2010){
  ...
  ### VERSION 3 EDITS
  exclude <- websites_exclude[websites_exclude$year == year, ]
  websites_exclude_list <- as.list(exclude$hostshort)
  couk <- couk[ !couk$host1short %in% websites_exclude_list , ]
  ...
  for (i in 1:LINKS){
    ...
    # version 4 edits (binary)
    # turn h to a 1 if h>0
    # turn h to a 0 if h=0
    h <- +(h>0)
  ...
  # save matrix A
  A_filepath_name = paste0("A_for_", year, "_binary2.mtx")
...
print(output)
#write.csv(output, file="2005-2010_makeAdjMatr_v3_info.csv")
