...
library(grid)
library(plotly)
setwd("~/Documents/MiniProject/Network_1_pc/15UKincScot")
...
# heatmap for each year when the A_LAD_binary data is Winsorized (0,0.99), then log10(A_wins+1)
AWinsLog <- vector(mode="list", length=length((years)))
for (i in 1:length(years)){
  ...
  A <- log10(A_wins+1)
  AWinsLog[[i]] = A # list of all winsorised and logged A matrices
}


### for the AWinsLog data:
allWinsLog <- do.call("cbind", AWinsLog) # make one rectangular matrix
SVD_WinsLog <- svd(allWinsLog)
...
## winsorize A by c(0, 0.99), then log10
PC1lim=range(SVD_WinsLog$v[,1])
PC2lim=range(SVD_WinsLog$v[,2])
PC3lim=range(SVD_WinsLog$v[,3])
PC4lim=range(SVD_WinsLog$v[,4])
PC5lim=range(SVD_WinsLog$v[,5])
...
animationData = cbind(SVD_WinsLog$v[ , 1:4] , rep(c(2005:2010), each=n))
...
write.csv(anim, "animationDataUK.csv")

####################################
## PLOT MAKING SECTION - 30/11/2022
####################################


pdf("screePlot_AWins1Log_titledPres_UK.pdf",
    width=8, height=3)
plot(SVD_WinsLog$d, pch=20, ylab="singular value", 
     main="Scree plot of Singular Values from SVD of Data for UK",
     col=ifelse(SVD_WinsLog$d < SVD_WinsLog$d[4], "black", "red")) # first 4 dots red
dev.off()

### map of UK coloured by 2D colourmap - filled
ggplot(LAD11_shp) + geom_sf(aes(geometry=centroids), 
                            fill = rgb(red=dat$Xscaled, blue=dat$Yscaled, green = 0.2), 
                            col=rgb(red=dat$Xscaled, blue=dat$Yscaled, green = 0.2),lwd=0.1) + 
  ggtitle("Filled map of LADs in the UK") 


### map of UK coloured by 2D colourmap - outline
ggplot(LAD11_shp) + geom_sf(aes(geometry=centroids), 
                            col=rgb(red=dat$Xscaled, blue=dat$Yscaled, green = 0.2),lwd=0.1) + 
  ggtitle("Outline map of LADs in the UK") 


## Embedding pLots

pdf("rgb_SVD_AWins1Log_year_PC3PC4_UK.pdf")
for (t in 1:length(years)){
  plot(SVD_WinsLog$v[n*(t-1)+1:n,3], SVD_WinsLog$v[n*(t-1)+1:n,4],
       xlab="PC3",ylab="PC4",
       col=rgb(red=dat$Xscaled, blue=dat$Yscaled, green = 0.2 ), 
       pch=20, cex=2, height=20, width=10,
       main=paste0(years[t], " Winsorized c(0, 0.99), log10 \nPC3 and PC4"),
...
for (t in 1:length(years)){
  plot(SVD_WinsLog$v[n*(t-1)+1:n,1], SVD_WinsLog$v[n*(t-1)+1:n,2],
       xlab="Dimension 1",ylab="Dimension 2",
       col=rgb(red=dat$Xscaled, blue=dat$Yscaled, green = 0.2 ), 
       pch=20, cex=2, height=20, width=10,
       main=paste0(years[t], " Winsorized c(0, 0.99), log10"),
       xlim=PC1lim,ylim=PC2lim)
  outliers = intersect(which(SVD_WinsLog$v[n*(t-1)+1:n,2] > 0.01), which(SVD_WinsLog$v[n*(t-1)+1:n,1] < -0.04))
  text(SVD_WinsLog$v[n*(t-1)+1:n,1][outliers], SVD_WinsLog$v[n*(t-1)+1:n,2][outliers],
       labels=rownames(plotdata)[outliers],cex=0.4, adj = c(-0.2,0))
  
  plot(SVD_WinsLog$v[n*(t-1)+1:n,2], SVD_WinsLog$v[n*(t-1)+1:n,3],
       xlab="Dimension 2",ylab="Dimension 3",
       col=rgb(red=dat$Xscaled, blue=dat$Yscaled, green = 0.2 ), 
       pch=20, cex=2, height=20, width=10,
       main=paste0(years[t], " Winsorized c(0, 0.99), log10"),
       xlim=PC2lim,ylim=PC3lim)
  outliers = which(SVD_WinsLog$v[n*(t-1)+1:n,2] > 0.045)
  text(SVD_WinsLog$v[n*(t-1)+1:n,2][outliers], SVD_WinsLog$v[n*(t-1)+1:n,3][outliers],
  ...
  numbers = c(1,7,10, 70)
  text(SVD_WinsLog$v[n*(t-1)+1:n,3][numbers], SVD_WinsLog$v[n*(t-1)+1:n,4][numbers],
       labels=rownames(plotdata)[numbers],cex=0.4, adj = c(-0.2,0))
  # minus sign in front of limits to rotate the plot 180 degrees
  
  plot(SVD_WinsLog$v[n*(t-1)+1:n,4], SVD_WinsLog$v[n*(t-1)+1:n,5],
